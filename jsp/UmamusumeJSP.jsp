...
<html>
	...
	<body>
		<button type="button" onclick="location.href='./'">ﾄｯﾌﾟﾍﾟｰｼﾞへ</button><button type="button" onclick="location.href='./RacingUmamusumeList'">ｷｬﾗｸﾀｰ一覧(実装順)ﾍﾟｰｼﾞへ</button><br>
		<button type="button" onclick="location.href='./en/UmamusumeList'">英語版ﾍﾟｰｼﾞへ</button>
		<h1>ウマ娘 キャラクター一覧</h1>
		...
		<div>
			...
			<form action="UmamusumeList" method="post">
				<input type="text" name="id" value="<%= id == null ? "" : id %>" placeholder="公式の図鑑番号(1～799)もしくは名前" title="公式の図鑑番号(1～799)もしくは名前/&quot;3桁までの数字&quot;もしくは&quot;20桁までのカタカナ(一部を除く)&quot;" size="26" pattern="(^[0-9０-９]{1,3}$)|(^[^\x01-\x7E]{0,20}$)">
			...
			<button type="button" onclick="location.href='./en/UmamusumeList'">英語版ﾍﾟｰｼﾞへ</button>
			<br>
			<span style="display: flex; justify-content: space-between;"><span><button type="button" onclick="location.href='./'">ﾄｯﾌﾟﾍﾟｰｼﾞへ</button><button type="button" onclick="location.href='./RacingUmamusumeList'">ｷｬﾗｸﾀｰ一覧(実装順)ﾍﾟｰｼﾞへ</button></span><span>App No.1, Version 5.5</span></span>
		</div>
	</body>
</html>