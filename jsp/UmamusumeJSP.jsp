...
<html>
	...
	<body>
		...
		<button type="button" onclick="location.href='../UmamusumeList'">Japanese Page/日本語版ﾍﾟｰｼﾞ</button>
		...
		<div>
			...
			<form action="UmamusumeList" method="post">
				<input type="text" name="id" value="<%= id == null ? "" : id %>" placeholder="Umadex No.1-799 or Name" title="1-3 digits (or 1-30 alphabetic characters)" size="30" pattern="(^[0-9０-９]{1,3}$)|(^[A-Za-z.]{0,30}$)">
			...
			<table>
				...
				<% for (UmamusumeBean u : umamusumeList) { %>
					<% if (u.umadexNo() < 800) { %>
				<tr>
					<td style="text-align: center;"><div class="div7" style="font-family: 'Oswald'">Umamusume</div><span class="dualFont"><%= u.umadexNo() %></span><% if (u.racingSuitNo() != 0) { %><br><div class="div5">(<%= NumberSuffix.addSuffix(u.racingSuitNo()) %> Trainee<br>Umamusume)</div><% } %></td><td><% if (u.parameter() != null) { %><a href="https://umamusume.com/character/<%= u.parameter() %>"><% } %><%= u.name() == null || u.name().startsWith("(Unknown") ? "&mdash;" : u.name() %><% if (u.parameter() != null) { %></a><% } %></td>
				</tr>
					<% } %>
					<% if (u.umadexNo() >= 800) { %>
				<tr>
					...
					<td style="text-align: center;" <% if (u.umadexNo() >= 800 && u.umadexNo() < 900) { %>class="div5"<% } %>><%= u.umadexNo() >= 800 && u.umadexNo() < 900 ? "Umamusume<br>as Tracen<br>Academy<br>Affiliate " + (u.umadexNo() - 800) : "<div class=\"div5\">Removed<br>Umamusume</div><span class=\"dualFont\">" + (u.umadexNo() - 900) + "</span>" %></td><td><% if (u.parameter() != null) { %><a href="https://umamusume.com/character/<%= u.parameter() %>"><% } %><%= u.name() == null || u.name().startsWith("(Unknown") ? "&mdash;" : nameWith2Lines %><% if (u.parameter() != null) { %></a><% } %></td>
				</tr>
					<% } %>
				<% } %>
			</table>
			...
			<table>
				...
				<tr>
					<td><% if (nu.parameter() != null) { %><a href="https://umamusume.com/character/<%= nu.parameter() %>"><% } %><%= nu.name() == null || nu.name().startsWith("(不明") ? "&mdash;" : nu.name() %><% if (nu.parameter() != null) { %></a><% } %></td>
				</tr>
				<% } %>
			</table>
			<br>
			...
			<button type="button" onclick="location.href='../UmamusumeList'">Japanese Page/日本語版ﾍﾟｰｼﾞ</button>
			<br>
			<span style="display: flex; justify-content: space-between;"><span><button type="button" onclick="location.href='./'">Top Page</button><button type="button" onclick="location.href='./RacingUmamusumeList'">Playable Character List</button></span><span>App No.2, Version 1.0</span></span>
		</div>
	</body>
</html>